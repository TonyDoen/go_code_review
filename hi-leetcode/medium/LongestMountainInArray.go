 ...
 */
func GetLongestMountainInArray0(arr []int) int {
	if nil == arr {
		return -1
	}
 ...
 */
func GetLongestMountainInArray1(arr []int) int {
	if nil == arr {
		return -1
	}
	result, up, down, length := 0, 0, 0, len(arr)
	for i := 1; i < length; i++ {
 ...
 */
func GetLongestMountainInArray2(arr []int) int {
	if nil == arr {
		return -1
	}
	result, length := 0, len(arr)
	for i := 0; i < length-1; {
		for i < length-1 && arr[i] >= arr[i+1] {
			i++
		}
		peak := i
		for peak < length-1 && arr[peak] < arr[peak+1] {
			peak++
		}
		j := peak
		for j < length-1 && arr[j] > arr[j+1] {
			j++
		}
		if i < peak && peak < j {
			tmp := j - i + 1
 ...
 */
func GetLongestMountainInArray3(arr []int) int {
	if nil == arr {
		return -1
	}
	result, length := 0, len(arr)
	for i := 1; i < length-1; i++ {
		if arr[i-1] < arr[i] && arr[i+1] < arr[i] {
			...
			for right < length-1 && arr[right] > arr[right+1] {
				right++
			}
			tmp := right - left + 1
			if tmp > result {

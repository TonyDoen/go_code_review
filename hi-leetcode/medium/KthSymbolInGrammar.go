 ...
 *
 */
func KthGrammar0(n, k int) int {
	if n < 1 || k < 1 || float64(k) > math.Pow(2.0, float64(n-1)) { // 入参错误
		return -1
	}
	if 1 == n {
		return 0
 ...
 */
func KthGrammar0Up(n, k int) int {
	if n < 1 || k < 1 || float64(k) > math.Pow(2.0, float64(n-1)) { // 入参错误
		return -1
	}
	if 1 == n {
	...
	return (1 & ^k) ^ KthGrammar0Up(n-1, (k+1)/2)
}
 ...
 */
func KthGrammar2Up(n, k int) int {
	if n < 1 || k < 1 || float64(k) > math.Pow(2.0, float64(n-1)) { // 入参错误
		return -1
	}
	return bits.OnesCount32(uint32(k-1)) % 2
}

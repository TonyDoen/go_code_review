 ...
 */
func FindReplaceString(src string, indexes []int, sources, targets []string) string {
	mp := make(map[int]int)
	for i, length := 0, len(indexes); i < length; i++ {
		sub := src[indexes[i] : indexes[i]+len(sources[i])]
		if sub == sources[i] {
			mp[indexes[i]] = i + 1
		}
	}
	var builder strings.Builder
	for i, length := 0, len(src); i < length; {
		idx := mp[i] - 1
		if idx >= 0 {
			builder.WriteString(targets[idx])
			i += len(sources[idx])
		} else {
			builder.WriteByte(src[i])
			i++
		}
	}

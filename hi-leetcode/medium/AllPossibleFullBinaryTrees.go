 ...
 */
func AllPossibleFullBinaryTree0(n int) *list.List {
	if 0 == n%2 { // 可以发现，所有的满二叉树的结点总数都是奇数
		return nil
	}
	result := list.New()
	if 1 == n {
	...
	for i := 1; i < n; i += 2 {
		...
		left := AllPossibleFullBinaryTree0(i)
		right := AllPossibleFullBinaryTree0(n - 1 - i)
 ...
 */
var mp = make(map[int]*list.List)

func AllPossibleFullBinaryTree1(n int) *list.List {
	// 可以发现，所有的满二叉树的结点总数都是奇数
	if 0 == n%2 {
		return nil
	}
	result := list.New()

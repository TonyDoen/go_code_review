 ...
 */
func FindAndReplacePattern0(words []string, pattern string) *list.List {
	result := list.New()
	for _, word := range words {
		w2p, p2w, i, n := make(map[byte]byte), make(map[byte]byte), 0, len(word)
		for i < n {
			iwc, ipc := word[i], pattern[i]
			w2pByte, p2wByte := w2p[iwc], p2w[ipc]
			if 0 != w2pByte && w2pByte != ipc {
				break
			}
			w2p[iwc] = ipc
			if 0 != p2wByte && p2wByte != iwc {
				break
			}
			p2w[ipc] = iwc
			i++
 ...
 *
 */
func FindAndReplacePattern1(words []string, pattern string) *list.List {
	result := list.New()
	for _, word := range words {
		w2p, p2w, i, n := make([]int, 26), make([]int, 26), 0, len(word)
		for i < n {
			iwc, ipc := word[i], pattern[i]
			wIdx, pIdx := iwc-'a', ipc-'a'
 ...
 */
func FindAndReplacePattern2(words []string, pattern string) *list.List {
	result := list.New()
	for _, word := range words {
		if helpFindAndReplacePattern2(word) == helpFindAndReplacePattern2(pattern) {
			result.PushBack(word)
		}
	}
	return result
}
func helpFindAndReplacePattern2(word string) string {
	mp, length := make(map[byte]int), len(word)
	for i := 0; i < length; i++ {
		ic := word[i]
		if 0 == mp[ic] {
			mp[ic] = len(mp) + 1

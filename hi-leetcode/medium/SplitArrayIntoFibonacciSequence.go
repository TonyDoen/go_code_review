 ...
 */
func SplitIntoFibonacci(s string) *list.List {
	result := list.New()
	helpSplitIntoFibonacci(result, 0, s)
	return result
}
func helpSplitIntoFibonacci(result *list.List, start int, s string) bool {
	length := len(s)
	if length == start && result.Len() >= 3 {
		return true
	}
	num := 0
	for i := start; i < length; i++ {
		if '0' == s[start] && i > start {
			break
		}
		...
		if result.Len() >= 2 {
			back1 := result.Back()
			back2 := back1.Prev()
			b1, b2 := back1.Value.(int), back2.Value.(int)
			if b1+b2 < num {
				break
			}
			if b1+b2 == num {
				result.PushBack(num)
				if helpSplitIntoFibonacci(result, i+1, s) {
					return true
				}
